...
class LineSegment():
    def __init__(self, groove, corner_points, iteration, display_line_segments=False, labels=np.zeros((1,1))):
        self.groove = groove
        ...
        for i in range(7):
            ...
            if avg_dist < 0.1:
                plt.clf()
                if len(labels) == 5:
                    plt.scatter(labels[:,0], labels[:,1], s=20, c='r')
                    #plt.scatter(self.orig[:,0], self.orig[:,1], s=20)
                plt.scatter(self.groove[0], self.groove[1], s=1)
                plt.scatter(self.corner_points[:,0], self.corner_points[:,1], s=20, c='g')
                plt.show()
    ...
    def divide_into_segments(self):
        idx = np.searchsorted(self.groove[0], self.corner_points.T[0], side="left")
        self.segments = [self.groove[:, idx[i]:idx[i+1] - 1] for i in range(len(idx)-1)]
    ...
    def segment_line_fit(self, index, iterations):
        ...
        #y_est_new = y_est + np.average(y_diff)
            ...
            if index == 1:
                ...
                if index == 3:
                    noise_threshold = 2 - 0.2 * iterations
                    limit = 0.5
                    reduction = 0.8
    ...
    def update(self, iterations):
        ...
        #    self.segment_line_fit(i,iterations)
        #print(self.line_segments, 'asdfasd')
        #self.line_segments = self.line_segments[self.line_segments[:, -1].argsort()]

        print()
        print()   

        #self.line_segments = self.line_segments[:,:-1]
        ...
        # the last corner tends to move without improvement, so reducing its importance
        self.dist_moved[-1] = self.dist_moved[-1] / 2 
        
        #avg_dist = np.average(np.linalg.norm(self.corner_points - new_corners, axis=1)[:-1])
...
if __name__ == '__main__':
    ...
    state_dict = torch.load('/home/oyvind/Blender-weldgroove/ResNet_NewSet80.pth')
    ...
    for groove in grooves[:]:
        ...
        # definer objekt
        line = LineSegment(cheiflip, hmm, 0, display_line_segments=False)
